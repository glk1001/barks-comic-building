...
def svgs_to_pngs(comics_database: ComicsDatabase, title_list: list[str]) -> None:
    ...
    for title in title_list:
        ...
        num_png_files += len(srce_svg_files)

    logger.info(f"\nTime taken to convert all {num_png_files} files: {int(time.time() - start)}s.")


def convert_svg_to_png(srce_file: Path, srce_svg: Path) -> None:
    ...
    try:
        ...
        if not srce_svg.is_file():
            msg = f'Could not find srce svg file: "{srce_svg}".'
            raise FileNotFoundError(msg)  # noqa: TRY301
        ...
        srce_width, srce_height = get_image_size(srce_file)
