...
class LedgerWriter:
    ...
    def __init__(self, ledger_file: Path, recipe: RestoreRecipe, workers: dict[str, int]) -> None:
        ...
        # The timestamp and pid alone are not unique - two runs started in the same
        # second, or a reused pid, would collide, and since runs are read back into a
        # dict keyed by this the later one would silently swallow the earlier. Losing a
        # record is the one thing an append-only ledger must not do, so a random suffix
        # settles it.
        self.run_id = f"{self.started}-{os.getpid()}-{uuid.uuid4().hex[:6]}"
    ...
    with path.open(encoding="utf-8") as f:
        for line_num, line in enumerate(f, start=1):
            ...
            try:
                ...
                if record.get("schema", 0) > LEDGER_SCHEMA:
                    msg = f"schema {record.get('schema')} is newer than {LEDGER_SCHEMA}"
